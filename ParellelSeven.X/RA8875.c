...
{	
    //UINT DOUT = cmd;//= (PORTC & 0x40);

    //DOUT |= (UINT)((UINT)((UINT)Data<< 1) & 0x180);
    //DOUT |= (UINT)(Data & 0x3F);

    RD_HIGH();Delay1us(1);
    LCD_CS_LOW();Delay1us(1);
    RS_HIGH();Delay1us(1);
...
unsigned char LCD_DataRead(void)
{
    ...
    DOUT = (uchar)(PORTC&0xFF);Delay1us(1);

    RD_HIGH();Delay1us(1);
    LCD_CS_HIGH();Delay1us(1);
    RS_HIGH();

    //lcd_data = (unsigned char)((DOUT & 0x180) >> 1);
    //lcd_data |= (unsigned char)(DOUT & 0x3F);
...
/*
void Write_DirW(uchar command, uint data)
{
    Write_Dir(command, data & 0xFF);
    Write_Dir(command+1, data >> 8);
}

void Write_Dir(unsigned char command, unsigned int data)
{
    CS_LOW(LCD);
